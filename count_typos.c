...
int main(int argc, char* argv[]) {
  ...
  char* typos_filename = (char*) malloc(1 + strlen(input_filename));
  // Checks is allocation fails
  if (typos_filename == NULL) {
    return EXIT_FAILURE;
  }
  snprintf(typos_filename, strlen(input_filename), "%s", input_filename);
  // strcat(typos_filename, TYPOS_SUFFIX);
  snprintf(typos_filename, MAX_WORD_LENGTH, "%s", TYPOS_SUFFIX);
  ...
  free(typos_filename);
  // Frees up the words in dictionary
  free_dictionary(dict, dict_size);
  // Frees dictionary itself
  free(dict);
  fclose(typos_output);
  fclose(stats_output);
  fclose(text);
